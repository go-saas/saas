...
// The TenantConnFunc type is an adapter to allow the use of ordinary
// function as TenantConn mutator.
type TenantConnFunc func(context.Context, *ent.TenantConnMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TenantConnFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TenantConnMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TenantConnMutation", m)
}
