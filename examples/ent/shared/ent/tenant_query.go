...
func (tq *TenantQuery) QueryConn() *TenantConnQuery {
	query := &TenantConnQuery{config: tq.config}
...
func (tq *TenantQuery) sqlAll(ctx context.Context) ([]*Tenant, error) {
	...
	if query := tq.withConn; query != nil {
		...
		for _, n := range neighbors {
			fk := n.tenant_conn
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "tenant_conn" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "tenant_conn" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Conn = append(node.Edges.Conn, n)
		}
	}
