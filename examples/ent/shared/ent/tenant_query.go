...
func (tq *TenantQuery) QueryConn() *TenantConnQuery {
	query := (&TenantConnClient{config: tq.config}).Query()
...
// WithConn tells the query-builder to eager-load the nodes that are connected to
// the "conn" edge. The optional arguments are used to configure the query builder of the edge.
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := tq.withConn; query != nil {
		if err := tq.loadConn(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Conn = []*TenantConn{} },
			func(n *Tenant, e *TenantConn) { n.Edges.Conn = append(n.Edges.Conn, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TenantQuery) loadConn(ctx context.Context, query *TenantConnQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantConn)) error {
	...
	query.withFKs = true
	query.Where(predicate.TenantConn(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.ConnColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.tenant_conn
		if fk == nil {
			return fmt.Errorf(`foreign-key "tenant_conn" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "tenant_conn" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
